...
public class decodeST {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Decoded message = "
                    + decodeMessage(pngImage).substring(0, decodeMessage(pngImage).indexOf(ENDING_MESSAGE)));
        }
    ...
    public static String decodeMessage(BufferedImage image) {
        ...
        int[] message = new int[height * width * 3]; // each pixel is 1 bit, so total bits is height * width. * 3 b/c 3
                                                     // bits/pix
        int count = 0;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                                           ...
                                           // method
                {
                    ...
                    message[count++] = decodeBit((pixelValue >> 16) & 0xFF); // Get the LSB of red
                    message[count++] = decodeBit((pixelValue >> 8) & 0xFF); // Get the LSB of green
                    message[count++] = decodeBit(pixelValue & 0xFF); // Get the LSB of blue
